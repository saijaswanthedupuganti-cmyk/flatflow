...
import habitiq.app.data.ExpenseSummary
...
data class HomeDashboardData(
    ...
    val activeTasks get() = tasks.filter { it.status == "pending" || it.status == "overdue" }
    val todaysTasks: List<FlatTask>
        get() {
            val today = LocalDate.now().toString()
            val filtered = activeTasks.filter { it.dueDate == today }
            return if (viewMode == HomeViewMode.MY) filtered.filter { it.currentAssignedUserId == currentUid } else filtered
        }
    val tasksTodayCount get() = activeTasks.count { it.dueDate == LocalDate.now().toString() }
    val completedThisWeek get() = tasks.count { it.status == "completed" && isWithinLastWeek(it.dueDate) }
    val weeklyProgress: Int
        get() {
            ...
            return ((weekTasks.count { it.status == "completed" }.toFloat() / weekTasks.size) * 100).toInt().coerceIn(0, 100)
        }
    private fun isWithinLastWeek(dateStr: String): Boolean = runCatching {
        ChronoUnit.DAYS.between(LocalDate.parse(dateStr), LocalDate.now()) in 0..7
    }.getOrDefault(false)
}
...
class HomeDashboardViewModel(
    ...
    fun load() {
        ...
        if (user == null) { _status.value = HomeDashboardStatus.NoFlat; return }
        _status.value = HomeDashboardStatus.Loading
        viewModelScope.launch {
            usersRepository.getActiveFlatId(user.uid).fold(
                onSuccess = { id -> if (id == null) _status.value = HomeDashboardStatus.NoFlat else observeFlat(id, user) },
                onFailure = { _status.value = HomeDashboardStatus.Error(it.message ?: "Couldn't load your flat.") }
            )
        }
    }
    ...
    private fun observeFlat(flatId: String, user: FirebaseUser) {
        ...
        observeJob = viewModelScope.launch {
            ...
            combine(
                ...
                expensesRepository.observeMonthlyExpenses(flatId, monthPrefix).catch { emit(ExpenseSummary(0.0, 0)) }
            ) { members, tasks, activity, expenses ->
                val flat = flatsRepository.getFlat(flatId).getOrNull()
                if (flat == null) HomeDashboardStatus.Error("Couldn't load flat details.")
                else HomeDashboardStatus.Ready(
                    HomeDashboardData(
                        flat = flat,
                        members = members,
                        tasks = tasks,
                        ...
                        currentUid = user.uid,
                        displayName = user.displayName?.substringBefore(" ") ?: user.email?.substringBefore("@") ?: "there",
