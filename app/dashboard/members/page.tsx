...
import {
  ...
  ClipboardList, UserCheck, UserX, TrendingUp,
} from 'lucide-react'

export default function MembersPage() {
  const { members, tasks, changeMemberStatus } = useFlatStore()
  ...
  // Count how many tasks each member is currently assigned to
            ...
            <Card key={member.uid} className={`shadow-sm overflow-hidden transition-shadow hover:shadow-md ${
              ...
              <CardContent className="p-5 space-y-4">
                ...
                {isAdmin && (
                  ...
                  >
                    {member.status === 'out_of_station' ? (
                      <><CheckCircle2 size={14} className="mr-2" /> Mark Available</>
                    ) : (
                      <><MapPinOff size={14} className="mr-2" /> Mark Out of Station</>
                    )}
                  </Button>
                )}
              </CardContent>
